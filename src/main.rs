...
fn main() {
    ...
    let rom_bin = match load_nesrom_file(path) {
        Ok(bin) => bin,
        Err(err) => {
    ...
    let nesrom = parse_nesrom(&rom_bin);
}
...
fn load_nesrom_file(path: &str) -> Result<Vec<u8>> {
...
fn parse_nesrom(rom: &Vec<u8>) -> Box<NesRom>
 {
    // NESファイルを読み込んで解析する
    ...
    if  header[0] != 0x4E ||
        ...
        header[3] != 0x1A
    {
        err_exit("this file is not NES format.");
    }
