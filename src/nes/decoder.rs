...
use crate::nes::executer as ex;

pub fn decode(cpu: &mut Cpu) -> u8 {
    // 当面は非公式命令を検出した場合にpanicさせる。
    let opcode = cpu.fetch();
    if let Some(wait) = decode_tier1(opcode, cpu) {
        return wait;
    }
    if let Some(wait) = decode_tier2(opcode, cpu) {
        return wait;
    }
    if let Some(wait) = decode_tier3(opcode, cpu) {
        return wait;
    }
...
/// OPコードの末尾2ビットを使った解析
fn decode_tier1(opcode: u8, cpu: &mut Cpu) -> Option<u8> {
    // "aaabbbcc" で分類
    ...
    if cc == 0b01 {
        let addr_mode = decode_addr_tier1_01(bbb);
...
fn decode_addr_tier1_01(bbb: u8) -> AddrMode {
    match bbb {
...
/// OPコードの末尾5ビットを使った解析
fn decode_tier2(opcode: u8, cpu: &mut Cpu) -> Option<u8> {
...
/// その他の1バイト命令をデコード
fn decode_tier3(opcode: u8, cpu: &mut Cpu) -> Option<u8> {
