...
impl Cpu {
    ...
    pub fn execute(&mut self) -> u8 {
        // 当面は非公式命令を検出した場合に panic させる。
        let opcode = self.fetch();
        if let Some(wait) = self.decode_by_last_2bit(opcode) {
            return wait;
        }
        if let Some(wait) = self.decode_by_last_5bit(opcode) {
            return wait;
        }
        if let Some(wait) = self.decode_remains(opcode) {
            return wait;
        }
    ...
    /// OPコードの末尾2ビットを使った解析
    pub fn decode_by_last_2bit(&mut self, opcode: u8) -> Option<u8> {
        // "aaabbbcc" で分類
        ...
        if cc == 0b01 {
            match aaa {
                ...
                */
                ...
                0b110 => {},    // CMP
                0b111 => {},    // SBC
                _ => panic!("\"{:0x}\" is undocumented opcode.", opcode),
            }
        } else if cc == 0b10 {
            match aaa {
                // アドレッシングモードに応じた実装を追加
    ...
    /// OPコードの末尾5ビットを使った解析
    pub fn decode_by_last_5bit(&mut self, opcode: u8) -> Option<u8> {
    ...
    pub fn decode_remains(&mut self, opcode: u8) -> Option<u8> {
